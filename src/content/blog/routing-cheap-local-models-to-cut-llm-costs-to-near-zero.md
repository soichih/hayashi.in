...
description: 'How I use a free local model as a delegated investigator for my AI assistant, trading wall-clock time for dollar cost — and what the research literature says about why this works.'
date: 2026-07-13
tags:
  - AI
  - LLM
...
I run a personal assistant using [OpenClaw](https://openclaw.ai). My primary model is `glm-5.2:cloud`, served through Ollama's cloud API — capable enough for conversation and synthesis, and far cheaper than a per-token frontier API. But it's still metered, and metered APIs have a way of quietly compounding. Debugging sessions are the worst offender: every turn re-sends the growing conversation, and a single multi-step troubleshooting session can rack up a real bill before you've fixed anything.
...
- **The orchestrator** — `glm-5.2:cloud`, which talks to me directly, makes decisions, and writes the final answer. For tasks that need frontier-level reasoning, I escalate explicitly to Claude Haiku or Opus.
...
There's an asymmetry worth knowing about if you try this yourself. Some cloud providers offer **prompt caching** — reusing a stable prefix of a conversation instead of reprocessing it from scratch, at a significant discount on the cached portion. That's a real, separate cost lever for keeping the orchestrator session itself cheap, and it's automatic — no architecture change required.
...
- **Judgment, synthesis, "what should I actually do about this" → keep on the orchestrator model, or escalate to Claude if it needs frontier-level reasoning.**
- **Escalate a specific delegated task to a Claude model explicitly** only when the local model's output comes back wrong, shallow, or it genuinely can't finish — not just because it's slow.

None of this eliminates the cloud model's bill entirely — I still pay for every message I send it and every summary it reads back. But it moves the *expensive* part of the work — the part that scales with how many things go wrong before you find the actual problem — onto hardware that's already paid for. For debugging-shaped tasks, which are inherently unpredictable in how many steps they'll take, that's exactly the kind of cost that's worth making free.
